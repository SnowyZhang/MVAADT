...
from .model_partial import Encoder_overall
from .preprocess import adjacent_matrix_preprocessing

class Train_MVAADT_partial():
    def __init__(self, 
        data,
        datatype = 'SPOTS',
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            self.model.train()
            results = self.model(self.features_omics1, self.features_omics2, self.adj_spatial_omics1, self.adj_feature_omics1, self.adj_spatial_omics2, self.adj_feature_omics2,self.data['adata_omics1'].uns['mask_index'],self.data['adata_omics1'].obsm['remain_index'])
        ...
        emb_omics1_recon = F.normalize(results['emb_recon_omics1'][self.data['adata_omics1'].uns['mask_index']], p=2, eps=1e-12, dim=1)
        ...
        output = {'emb_latent_omics1': emb_omics1.detach().cpu().numpy(),
                  'emb_latent_omics1_recon': emb_omics1_recon.detach().cpu().numpy(),
    ...
    def compute_loss(self, results):
        ...
        # reconstruction loss
        loss_recon_omics1 = F.mse_loss(self.features_omics1, results['emb_recon_omics1'][self.data['adata_omics1'].obsm['remain_index']])
        ...
        loss_translated_omics2_to_omics1 = F.mse_loss(results['emb_latent_omics1'], results['emb_latent_translated_omics2_to_omics1'][self.data['adata_omics1'].obsm['remain_index']])
        
        # alignment loss
        results = {k: v.to(self.device) for k, v in results.items()}
        real_labels1 = torch.ones(results['emb_latent_omics1_new'].size(0), 1).to(self.device)
        ...
        fake_labels2 = torch.zeros(results['emb_latent_omics1_new'].size(0), 1).to(self.device)
        ...
        return loss_recon_omics1, loss_recon_omics2, loss_omics1_ref, loss_omics2_ref,0,loss_translated_omics2_to_omics1
